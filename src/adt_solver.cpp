...
bool complex_adt_solver::asserted_eq(justification just, term_instance* lhs, term_instance* rhs, bool isTrue) const {
    ...
    if (propagator().is_adt_split()) {
        res = isTrue
                ? propagator().is_smt()
                    ? lhs->t->solver.unify_split(lhs, rhs, std::move(just))
                    : lhs->t->solver.unify(lhs, rhs, just)
                : lhs->t->solver.non_unify_split(lhs, rhs, just);
    }
    else {
        res = isTrue
              ? lhs->t->solver.unify(lhs, rhs, just)
              : lhs->t->solver.non_unify(lhs, rhs, just);
    }
...
literal complex_adt_solver::make_equality_atom(term_instance* lhs, term_instance* rhs) {
    ...
    if (!tryGetValue(eqToExpr, eq, expr)) {
        if (propagator().is_smt()) {
            auto lhsExpr = lhs->to_z3_us();
            auto rhsExpr = rhs->to_z3_us();
            auto eqExpr = lhsExpr == rhsExpr;
            assert(propagator().get_z3_propagator() != nullptr);
            expr = prop->m.mk_lit(propagator().get_z3_propagator()->register_observed_var(eqExpr, eq.to_string()));
        }
        else {
            expr = prop->m.mk_lit(prop->new_observed_var(OPT(eq.to_string())));
            eq.just.push_literal((literal)expr);
        }
...
formula complex_adt_solver::make_equality_expr(term_instance* lhs, term_instance* rhs) {
    ...
    for (const auto& eq2 : subproblems) {
...
void complex_adt_solver::make_z3_us(z3::context& ctx) {
    for (unsigned i = 0; i < solvers.size(); i++) {
        solvers[i]->z3USSort = ctx.uninterpreted_sort((get_solver_name(i) + "_US").c_str());
    }
}
...
void complex_adt_solver::make_z3_adt(z3::context& ctx) {
    ...
    for (unsigned i = 0; i < solvers.size(); i++) {
        ...
        constructor_names.push_back(Z3_mk_string_symbol(ctx, (get_solver_name(i) + "_ADT").c_str()));
    ...
    for (unsigned i = 0; i < n; i++) {
        solvers[i]->z3ADTSort = z3::sort(ctx, n_res[i]);
    }
...
bool simple_adt_solver::unify_split(term_instance* lhs, term_instance* rhs, justification just) {
    ...
    assert(propagator().is_smt() || r1->origin == nullptr || r1->origin->value == sat);
    assert(propagator().is_smt() || r2->origin == nullptr || r2->origin->value == sat);
    ...
    if (r1->t->is_const() && r2->t->is_const()) {
        ...
        for (unsigned i = 0; i < r1->t->Args.size(); i++) {
            formula f = complexSolver.make_equality_expr(r1->get_arg(i, propagator()), r2->get_arg(i, propagator()));
            if (f->is_true())
                continue;
            propagate(just, f);
        }
    ...
    if (merge_root(r1, r2, { lhs, rhs, { just } })) {
        return true;
    }
    just.push_equality(lhs, r1);
    just.push_equality(rhs, r2);
...
bool simple_adt_solver::non_unify_split(term_instance* lhs, term_instance* rhs, const justification& just) {
    switch (test_non_unify_split(lhs, rhs, just)) {
        case sat:
            return true;
        case unsat:
            return false;
...
tri_state simple_adt_solver::test_non_unify_split(term_instance* lhs, term_instance* rhs, justification just) {
...
bool simple_adt_solver::unify(term_instance* lhs, term_instance* rhs, justification& just) {
    ...
    if (r1->t->is_const() && r2->t->is_const()) {
        ...
        for (unsigned i = 0; i < r1->t->Args.size(); i++) {
            if (!unify(r1->get_arg(i, propagator()), r2->get_arg(i, propagator()), just))
                return false;
        }
        just.pop_equality();
        just.pop_equality();
...
z3::check_result simple_adt_solver::non_unify(Lazy* lazy) {
    ...
    while (lazy->LHS != nullptr || !lazy->prev.empty()) {
        ...
        if (r1 == r2) {
            lazy->just.push_equality(lazy->LHS, lazy->RHS);
            lazy->LHS = nullptr;
            lazy->RHS = nullptr;
            continue;
        }
        ...
        lazy->just.push_equality(lazy->LHS, r1);
        lazy->just.push_equality(lazy->RHS, r2);
...
bool simple_adt_solver::check_containment_cycle(term_instance* inst, justification& just) {
    if (!inst->t->is_const() || inst->t->is_ground())
        return true;
    auto* r = inst->find_root(propagator());
    just.push_equality(inst, r);
    for (const auto* arg : inst->t->Args) {
        if (!check_containment_cycle(arg->get_instance(inst->cpy_idx(), propagator()), r, just))
            return false;
    }
    just.pop_equality();
    return true;
...
bool simple_adt_solver::check_containment_cycle(term_instance* inst, term_instance* search, justification& justifications) {
    ...
    if (r == search) {
        // Found the cycle
        justifications.push_equality(inst, r);
    ...
    justifications.pop_equality();
    return true;
}

bool simple_adt_solver::non_unify(term_instance* lhs, term_instance* rhs, const justification& just) {
    // return true;
    Lazy* lazy = new Lazy(lhs, rhs, just);
...
tri_state simple_adt_solver::test_less(literal just, term_instance* lhs, term_instance* rhs) {
    ...
    if (r1->t->is_const() && r2->t->is_const()) {
        if (r1->t->FuncID != r2->t->FuncID) {
            ...
            justification justList;
            justList.push_literal(just);
            justList.push_equality(lhs, r1);
            justList.push_equality(rhs, r2);
            conflict(justList);
            return unsat;
        }
...
bool simple_adt_solver::check_smaller_cycle(term_instance* start, term_instance* startRoot, term_instance* current, justification& just) {
    ...
    if (startRoot == currentRoot) {
        just.push_equality(start, current);
        return false;
    }
    ...
    if (startRoot->t->is_const() && currentRoot->t->is_const() &&
        ...
        startRoot->t->FuncID < currentRoot->t->FuncID) {
        just.push_equality(start, startRoot);
        just.push_equality(current, currentRoot);
        return false;
    }
    ...
    do {
        ...
        for (auto i = 0; i < cnt; i++) {
            ...
            just.push_equality(current, current2);
            just.push_literal(justifications);
...
bool simple_adt_solver::check_diseq_replacement(term_instance* newRoot) {
    ...
    do {
        ...
        for (unsigned i = current->diseq_split_watches_idx; i < current->diseq_split_watches.size(); i++) {
            ...
            switch (test_non_unify_split(watch.LHS, watch.RHS, watch.just)) {
                case unsat:
                    return false;
                case undef:
                    break;
